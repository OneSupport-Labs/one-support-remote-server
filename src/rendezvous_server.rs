...
impl RendezvousServer {
    ...
    async fn handle_udp(
        ...
        if let Ok(msg_in) = RendezvousMessage::parse_from_bytes(bytes) {
            if let Some(ref union) = msg_in.union {
                log::info!("Received UDP msg: {:?} from {}", union, addr);
            }
            match msg_in.union {
    ...
    async fn handle_tcp(
        ...
        if let Ok(msg_in) = RendezvousMessage::parse_from_bytes(bytes) {
            if let Some(ref union) = msg_in.union {
                log::info!("Received TCP msg: {:?} from {}", union, addr);
            }
            match msg_in.union {
                Some(rendezvous_message::Union::PunchHoleRequest(ph)) => {
                ...
                Some(rendezvous_message::Union::RequestRelay(mut rf)) => {
                    let token = rf.custom_access_token.clone();
                    let username = rf.username.clone();
                    if !token.is_empty() {
                        log::info!("Verifying custom token for RelayRequest from {}: {}", addr, token);
                        match Self::verify_token(&token, &username, &rf.id).await {
                             TokenVerification::Valid => {
                                 log::info!("Token verification SUCCESS for RelayRequest from {}", addr);
                             }
                             _ => {
                                 log::warn!("Invalid/Expired custom token in RelayRequest from {}: {}", addr, token);
                                 return true;
                             }
                        }
                    } else {
                        log::warn!("Empty custom token in RelayRequest from {}", addr);
                         return true;
                    }
    ...
    async fn handle_punch_hole_request(
        ...
        let token = ph.custom_access_token.clone();
        if !token.is_empty() {
             log::info!("Verifying custom token from {}: {}", addr, token);
             let username = ph.username.clone();
             match Self::verify_token(&token, &username, &ph.id).await {
                TokenVerification::Valid => {
                    log::info!("Token verification SUCCESS for {}", addr);
                }
                TokenVerification::Expired => {
                    log::warn!("Expired custom token from {}: {}", addr, token);
                    let mut msg_out = RendezvousMessage::new();
                    msg_out.set_punch_hole_response(PunchHoleResponse {
                        failure: punch_hole_response::Failure::TOKEN_EXPIRED.into(),
                        ..Default::default()
                    });
                    return Ok((msg_out, None));
                }
                _ => {
                    log::warn!("Invalid custom token from {}: {}", addr, token);
                    let mut msg_out = RendezvousMessage::new();
                    msg_out.set_punch_hole_response(PunchHoleResponse {
                        failure: punch_hole_response::Failure::TOKEN_INVALID.into(),
                        ..Default::default()
                    });
                    return Ok((msg_out, None));
                }
             ...
             log::warn!("Empty custom token from {}", addr);
             let mut msg_out = RendezvousMessage::new();
             msg_out.set_punch_hole_response(PunchHoleResponse {
    ...
    async fn verify_token(token: &str, username: &str, id: &str) -> TokenVerification {
        let client = reqwest::Client::builder()
            .danger_accept_invalid_certs(true)
            .build()
            .unwrap_or_else(|_| reqwest::Client::new());
        let res = client
            .post("http://172.17.0.1:8080/api/auth/verify")
            .header("Authorization", format!("Bearer {}", token))
            .json(&serde_json::json!({ "username": username, "id": id }))
            .send()
            .await;
        match res {
            Ok(response) => {
